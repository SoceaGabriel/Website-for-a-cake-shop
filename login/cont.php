...
<meta name="description" content="Pagina unde utilizatorul isi poate vedea datele personale!">
...
<div id="cont_utilizator">
<h1>&#10036; Profilul meu</h1>
<img src="../imagini/user_logo.png" width="250px" height="250px" alt="User Logo" />
<!--Cod care afiseaza informatiile personale utilizatorului logat pe site-->
<?php
	echo "<h1>Bine ai venit pe acest site, <span style='color:magenta;font-style:italic; font-weight: bold;'>".$_SESSION['utilizator']."</span> !";
?>
<?php
	$nume_fisier="../utilizatori/utilizatori.txt";
	$identificator_fisier_utilizatori=@fopen($nume_fisier,"r");
	$ok=0;
	$utilizator=$_SESSION['utilizator'];
	$parola=$_SESSION['parola'];
	while (($buffer = fgets($identificator_fisier_utilizatori, 4096)) !== false) {
	...
	if($ok==1)
	{
		$buffer = fgets($identificator_fisier_utilizatori, 4096);
		$buffer_1=fgets($identificator_fisier_utilizatori, 4096);
		echo "<h1>Numele dvs: <span style='color:magenta;font-style:italic; font-weight: bold;'>".$buffer_1." ".$buffer."</span></h1>";
		$buffer = fgets($identificator_fisier_utilizatori, 4096);
		echo "<h1>Adresa dvs: <span style='color:magenta;font-style:italic; font-weight: bold;'>".$buffer."</span></h1>";
		$buffer = fgets($identificator_fisier_utilizatori, 4096);
		echo "<h1>Email-ul dvs: <span style='color:magenta;font-style:italic; font-weight: bold;'>".$buffer."</span></h1>";
		$buffer = fgets($identificator_fisier_utilizatori, 4096);
		echo "<h1>Numarul de telefon: <span style='color:magenta;font-style:italic; font-weight: bold;'>".$buffer."</span></h1>";
	}
?>
<div id="latime_linkuri">
<a class="bloc_welcome" href="../produse/torturi_nunta.php">Cumpără un produs</a><br>
<form action="logout.php" class="bloc_welcome" method="post"><center><input class="bloc_welcome" type="submit" name="logout" value="Deconectare"/></center></form><br/>
</div>
</div>
</div>
